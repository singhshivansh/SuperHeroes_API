...
app.get('/get_marvel_movie', (req, res)=>{
    ...
    let film;
    if (movie_name)
        film = mar.filter(e=>{return e.name.toLowerCase().includes(movie_name.toLowerCase())});
    if (actor_name)
        film = mar.filter(e=>{return e.stars[0].toLowerCase().includes(actor_name.toLowerCase())});
